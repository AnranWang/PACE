		...
		for i in range(len(self.train_data['label'])):
			...
			if user in self.user_label:
				...
				for j in user_context:
					...
					##data.append(1)
					tmp[j] = 1
				#self.train_data['user_context'].append(self.user_label[user])#
			##self.context_data['user_context'].append(csr_matrix((data, (row_ind, col_ind)), shape=(1, len(self.user_enum))))
			self.context_data['user_context'].append(np.array(tmp))
			#else:#
				#self.train_data['user_context'].append([])#
			...
			if spot in self.spot_label:
				...
				for j in spot_context:
					...
					##data.append(1)
					tmp[j] = 1
				#self.train_data['spot_context'].append(self.spot_label[spot])#
			##self.context_data['spot_context'].append(csr_matrix((data, (row_ind, col_ind)), shape=(1, len(self.spot_enum))))
			self.context_data['spot_context'].append(np.array(tmp))
			#else:
				#self.train_data['spot_context'].append([])
